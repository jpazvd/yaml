...
```stata
net install yaml, from("https://raw.githubusercontent.com/jpazvd/yaml/main/src/") replace
```

That is the whole install: 25 files, no dependencies. They go to whichever
directory `net set ado` currently points at — `PLUS` unless you have changed it.
`replace` lets the same line upgrade an existing copy, so it is safe to re-run.

To see what you are about to install first:

```stata
net from "https://raw.githubusercontent.com/jpazvd/yaml/main/src/"
net describe yaml
```

**Pin a version for reproducible work.** Swapping `main` for a release tag
installs exactly that release and nothing later, which is what a replication
package or a shared pipeline wants:

```stata
net install yaml, from("https://raw.githubusercontent.com/jpazvd/yaml/v2.0.1/src/") replace
```
...
The header line it prints carries the version and build date. To remove the
package: `ado uninstall yaml`.
...
```stata
ssc install github
github install jpazvd/yaml, path(src)
```
...
`stata.toc` and `yaml.pkg` under `src/`, not at the repository root. Without the
option the command looks for `stata.toc` at the root and stops with
`file ... not found` (r(601)). The `master` in that URL is not a problem —
GitHub serves the default branch under that name.
...
Stata 14.0 for core functionality. Stata 16.0 or later is required for anything
that uses frames: the `yaml frames` subcommand, and the `frame()`, `cache()` and
`index()` options of `yaml read`. Mata is used by the bulk parser and is part of
Stata, not a separate install.

The nine subcommands are `read`, `write`, `describe`, `list`, `get`, `validate`,
`dir`, `frames` and `clear`. There is no `yaml cache` or `yaml index` — those are
option names.

**There are no required external packages.** Nothing in `src/` installs or calls
another user-written command, which is why `github install` reports
`yaml package has no dependency` and why no `dependency.do` ships with the
package.

One package is *optional* and guarded:

| Package | When | Behaviour if absent |
| --- | --- | --- |
| `dta2ddi` | only when a do-file generated by `yaml_harmonize` is run with `ddiexport` | the generated file guards the call with `capture which dta2ddi` and prints a skip message; nothing fails |

Two packages are sometimes mistaken for dependencies. `wbopendata` and
`unicefdata` are named in the help file as examples of packages whose YAML
catalogs `yaml` can read, and the supplementary example `yaml_catalog_integration.do`
needs them installed. The package itself does not: it neither calls nor requires
them.
...
├── qa/
│   ├── run_tests.do       # QA runner (36 tests)
