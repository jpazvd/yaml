# YAML Fast-Read & Cache Plan (Speed-First)

**Date:** 04Feb2026  
**Owner:** João Pedro Azevedo  
**Status:** Draft (performance-first design)  
**Primary Consumer:** wbopendata (Stata)  
**Goal:** Deliver speed-first parsing for large YAML metadata while preserving canonical YAML correctness. Achieve this via (1) canonical parser optimizations and caching, and (2) an opt-in fast-read path for wbopendata-class workloads.
...
- **Opt-in fast-read** for near-infix speed on large, regular YAML subsets.
...
2. **Single pass file read** in fast-read mode.
3. **Field-selective extraction** (avoid parsing the entire file).
4. **Cacheability** at the dataset level (frame cache for Stata 16+).
5. **Backward-compatible API**: existing `yaml read` must remain unchanged unless fast-read is explicitly requested.
...
- **Fast-Read Parse (opt-in):** Lightweight line scanner with a restricted YAML subset.

**Why dual-path?** Canonical parsing stays correct and general; fast-read provides speed for constrained schemas. Optimizations make canonical parsing faster without changing semantics.
...
1. **Selective extraction**
  - Add `fields()` to standard parsing as well (not only fastread).
...
### 4.1 New Option: `fastread`

A new option for `yaml read`:

```
yaml read using "file.yaml", fastread
```

Fast-scan mode can optionally accept field filters:

```
yaml read using "file.yaml", fastread fields(name description source_id topic_ids)
```
...
Example:
```
yaml read using "_wbopendata_indicators.yaml", fastread fields(name description source_id topic_ids)
```
...
Example:
```
yaml read using "_wbopendata_indicators.yaml", fastread listkeys(topic_ids topic_names)
```
...
```
yaml read using "_wbopendata_indicators.yaml", fastread fields(...) cache(frame=_yaml_ind_cache)
```
...
## 5. Checks & Error Messages (Canonical vs Fast-Read)
...
### 5.3 Fast-Read Checks

- **Unsupported YAML feature** (anchors, aliases, nested mappings, block scalars when disabled)
  - Error: `fastread unsupported YAML feature at line <n>. Rerun without fastread.` (exit 198)
- **List key requested but list not found**
  - Return empty rows (no error)
- **`listkeys()` without `fastread`**
  - Error: `listkeys() requires fastread` (exit 198)
- **`fastread` with unsupported option combos** (e.g., `fastread` + deep path selectors)
  - Error: `fastread does not support nested path selectors` (exit 198)

---

## 6. Minimal Output Schema (Fast-Read Mode)
...
### Phase 2: Fast-Read Prototype (Opt-in)
- Add `fastread` API and `listkeys()` support.
- Add frame caching for fast-read results (Stata 16+).
- Add a hidden option in wbopendata to toggle yaml fastread.
...
- If not, switch wbopendata to fast-read path (keep canonical default for general users).
...
- **Stata 11–15:** fastread works, no caching
...
| Maintenance complexity | Medium | Keep fastread code path compact and documented |
...
4. Implement `fastread` + `listkeys()`
5. Add fast-read frame caching
6. Add benchmark script comparing infix vs canonical-optimized vs fastread
