...
\begin{abstract}
YAML (``YAML Ain't Markup Language'') is a human-readable data-serialization standard used widely for configuration files, structured metadata, and declarative workflows. It underpins many modern analytical ecosystems, including GitHub Actions, Python and R pipelines, Jupyter-based workflows, and cloud and container orchestration tools. Despite Stata's increasing role in multi-language reproducible analytics, it currently lacks native support for parsing or generating YAML. This article introduces \texttt{yaml}, a unified Stata command with nine subcommands---\texttt{read}, \texttt{write}, \texttt{describe}, \texttt{list}, \texttt{get}, \texttt{validate}, \texttt{dir}, \texttt{frames}, and \texttt{clear}---for reading, writing, querying, and validating YAML files. The command enables users to import structured YAML configuration files into Stata datasets or frames and export Stata metadata back into YAML format. The implementation is compact, has no external dependencies, and targets the JSON-compatible subset of the YAML~1.2 specification. We illustrate applications including API-driven data downloads using YAML-stored indicator metadata and YAML-driven survey microdata harmonization with variable and value mappings. We conclude with recommendations for integrating YAML-based workflows into Stata-centered data production systems.
...
YAML is a widely used human-readable data-serialization format designed for configuration files, structured metadata, and declarative workflows \citep{YAML12}. It is now ubiquitous across analytical and data engineering platforms, including R Markdown and Quarto site configuration files, GitHub Actions workflows, Python configuration files, and cloud and container orchestration templates. YAML's broad adoption is due in part to its expressiveness, readability, and close relationship to JSON.

Stata, in contrast, currently has no built-in mechanism to read or write YAML structures. As empirical workflows increasingly span multiple programming environments, the absence of YAML support can restrict Stata's ability to participate in modern reproducible pipelines. In practice, this often leads to duplicated configuration logic, manual copy-and-paste of settings, and difficulties integrating Stata into automated systems that assume YAML as a configuration lingua franca.

This article introduces \texttt{yaml}, a unified Stata command that provides a complete interface for YAML file operations through nine subcommands:
\begin{center}
\begin{tabular}{ll}
\texttt{yaml read}     & parse YAML file into Stata dataset or frame \\
\texttt{yaml write}    & export dataset or frame back to YAML format \\
\texttt{yaml describe} & display structure of loaded YAML data \\
\texttt{yaml list}     & list keys, values, or children of a key \\
\texttt{yaml get}      & retrieve attributes of a specific key \\
\texttt{yaml validate} & validate required keys and types \\
\texttt{yaml dir}      & list all YAML data in memory (dataset and frames) \\
\texttt{yaml frames}   & list only YAML frames in memory (Stata 16+) \\
\texttt{yaml clear}    & clear YAML data from memory
\end{tabular}
\end{center}

The objective is not full YAML-specification compliance but a robust and pragmatic subset sufficient for most analytical workflows encountered in practice. The design emphasizes simplicity, transparency, portability, and the absence of external runtime dependencies. We show that even a small JSON-compatible subset of the YAML~1.2 standard is adequate to support configuration-driven empirical research, automated validation workflows, and cross-language reproducibility.

The remainder of the article is organized as follows. Section~\ref{sec:motivation} motivates the need for YAML interoperability in Stata-based workflows. Section~\ref{sec:architecture} describes the command architecture and data model. Section~\ref{sec:subcommands} presents each subcommand in detail. Section~\ref{sec:applications} illustrates practical applications in data production pipelines and validation frameworks. Section~\ref{sec:discussion} discusses limitations and possible extensions. Section~\ref{sec:conclusions} concludes.
...
\begin{aboutauthor}
Jo\~ao Pedro Azevedo is Deputy Director and Chief Statistician in UNICEF's Division of Data, Analytics, Planning and Monitoring. His work focuses on official statistics, survey and administrative data systems, and reproducible analytics for monitoring child-related outcomes.
\end{aboutauthor}
