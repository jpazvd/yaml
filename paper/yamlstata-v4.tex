% yamlstata-v4.tex — Stata Journal insert
% To be included from main-v4.tex provided by sjlatex
% Version 1.7.0 with Phase 2 features (Mata bulk parser, collapse, strL)
...
. yaml read using filename [, frame(name) locals scalars
      ...
      stream index(string) blockscalars bulk collapse strl]
\end{verbatim}
...
\textbf{Phase~2 options} (high-performance Mata parser)
\begin{itemize}
    ...
    \item \texttt{collapse} produces wide-format output with one row per top-level key. After parsing with \texttt{bulk}, run the \texttt{\_yaml\_collapse} helper to reshape the data into a format where each indicator (or top-level entry) occupies a single row with fields as columns.
...
. yaml read using "indicators.yaml", bulk blockscalars strl replace
(29,189 rows read via Mata bulk parser)

. _yaml_collapse
(collapsed to 830 indicators, wide format)
\end{stlog}
...
\textbf{Scaling to production catalogs.} Real-world indicator registries operate at substantially larger scales. The \texttt{wbopendata} package \citep{azevedo2014wbopendata} maintains a catalog of approximately 29,000 World Bank indicators stored in YAML files exceeding 360,000 lines, while \texttt{unicefdata} \citep{azevedo2024unicefdata} provides access to 800+ UNICEF indicators. These catalogs benefit from the Phase~2 optimizations:

\begin{stlog}
. yaml read using "wdi_catalog.yaml", frame(wdi) bulk collapse
. frame wdi: count
    29,189

. yaml read using "unicef_catalog.yaml", frame(unicef) bulk collapse strl
. frame unicef: count
    830
\end{stlog}

The \texttt{bulk} option enables streaming tokenization that processes these large files without memory spikes, while \texttt{collapse} converts the flattened key-value pairs into a wide-format dataset suitable for direct querying. For catalogs with long description fields, the \texttt{strl} option stores values as strLs, avoiding Stata's 2,045-character string limit. This combination makes it practical to load, filter, and query indicator metadata at scales previously impractical in Stata. To ensure that future \texttt{yaml} updates do not break these production workflows, compatibility tests covering frame-based query patterns are maintained as part of the QA framework (see Appendix~\ref{sec:qa}).
...
\begin{itemize}
    ...
    \item interactive tools for inspecting or editing YAML directly within Stata; and
    \item integration with frame-based query operations for wbopendata-style indicator discovery.
\end{itemize}
...
The QA system comprises four test families covering distinct aspects of functionality:
...
FEAT  & Features       & 8  & New functionality validation \\
\hline
      & \textbf{Total} & 22 & \\
\hline
\end{tabular}
...
. do run_tests.do         // Full suite (22 tests)
...
As of v1.7.0, all 22 tests pass on Stata 17+ and Stata 18. Frame-dependent tests (REG-02, FEAT-08) require Stata~16+; they are automatically skipped on earlier versions with appropriate warnings.
